...
const MAJOR_HUBS = ["Dhaka", "Chattogram", "Sylhet", "Rajshahi", "Khulna", "Barishal", "Rangpur", "Mymensingh", "Jashore", "Bogura", "Cumilla", "Feni", "Bhairab", "Dinajpur", "Panchagarh", "Dohajari"];
...
const TRAIN_ROUTES: TrainRouteEntry[] = [
    ...
    {
        from: "Dohajari", to: "Cox's Bazar",
        trains: ["Cox's Bazar Express", "Parjotak Express", "Commuter Train"],
        fare: "100-300", time: "1.5-2h",
        shovan: "100", snigdha: "200", acSeat: "—", acBerth: "—"
    },
    {
        from: "Satkania", to: "Cox's Bazar",
        trains: ["Cox's Bazar Express", "Parjotak Express", "Commuter Train"],
        fare: "80-250", time: "1h",
        shovan: "80", snigdha: "180", acSeat: "—", acBerth: "—"
    },
    {
...
const BUS_ROUTES_MAJOR = [
    ...
    { from: "Bandarban", to: "Dohajari", buses: ["Local buses", "Shared Jeep"], fare: "100-200" },
    { from: "Bandarban", to: "Satkania", buses: ["Local buses", "Shared Jeep"], fare: "100-200" },
...
const AIRPORT_MAP: { [key: string]: { city: string, airport: string, bnAirport: string } } = {
    ...
    "Bandarban": { city: "Chattogram", airport: "Shah Amanat International Airport", bnAirport: "শাহ আমানত আন্তর্জাতিক বিমানবন্দর" },
    "Dohajari": { city: "Cox's Bazar", airport: "Cox's Bazar Airport", bnAirport: "কক্সবাজার বিমানবন্দর" },
    "Satkania": { city: "Cox's Bazar", airport: "Cox's Bazar Airport", bnAirport: "কক্সবাজার বিমানবন্দর" },
};
...
const buildViaHubSuggestions = (from: string, to: string): Array<{
    ...
    const directDist = (coordFrom && coordTo) ? calculateDistance(coordFrom[0], coordFrom[1], coordTo[0], coordTo[1]) : 0;

    for (const hub of MAJOR_HUBS) {
        if (hub === from || hub === to) continue;

        const coordHub = DISTRICT_COORDINATES[hub];
        const dist1 = coordFrom && coordHub ? calculateDistance(coordFrom[0], coordFrom[1], coordHub[0], coordHub[1]) : 0;
        const dist2 = coordHub && coordTo ? calculateDistance(coordHub[0], coordHub[1], coordTo[0], coordTo[1]) : 0;
        const totalDist = dist1 + dist2;

        // --- HEURISTIC: Avoid Logical Detours ---
        // 1. If detour is > 1.8x original distance and original is reasonably short, ignore it.
        if (directDist > 0 && directDist < 250 && totalDist > directDist * 1.8) continue;
        // 2. If detour adds more than 300km, skip unless direct is very long.
        if (totalDist > directDist + 300 && directDist < 500) continue;
        // 3. Special case: Chittagong Hill Tracts to nearby coastal areas (shouldn't go to Dhaka)
        const hillTracts = ['Bandarban', 'Rangamati', 'Khagrachari'];
        if (hillTracts.includes(from) && to === "Cox's Bazar" && hub === "Dhaka") continue;
    ...
    if (isBn) {
        ...
        if (!busInfo && brtcRoutes.length === 0 && routesToDisplay.length === 0) {
            ...
            if (isHillTract) {
                ...
                if (hillCity === 'Bandarban' && to === "Cox's Bazar") {
        ...
        if (viaHubs.length > 0) {
            ...
            for (const { hub, leg1Modes, leg2Modes, leg1Time, leg2Time } of viaHubs) {
                // If both legs have the same mode (e.g. Bus + Bus), simplify it
                const sharedModes = leg1Modes.filter(m => leg2Modes.includes(m));
                
                if (sharedModes.includes('Bus')) {
                    result += `🚗 **সড়কপথে:** ${from} → ${hub} (বাস${leg1Time ? ` ~${leg1Time}` : ''}) তারপর ${hub} → ${to} (বাস${leg2Time ? ` ~${leg2Time}` : ''})  \n`;
                }
                
                if (sharedModes.includes('Train')) {
                    result += `🚂 **ট্রেনযোগে:** ${from} → ${hub} (ট্রেন${leg1Time ? ` ~${leg1Time}` : ''}) তারপর ${hub} → ${to} (ট্রেন${leg2Time ? ` ~${leg2Time}` : ''})  \n`;
                }

                if (leg1Modes.includes('Flight') || leg2Modes.includes('Flight')) {
                    const l1 = leg1Modes.map(m => modeNameBn[m] ?? m).join('/');
                    const l2 = leg2Modes.map(m => modeNameBn[m] ?? m).join('/');
                    result += `🛫 **আকাশপথে:** ${from} → ${hub} (${l1}) তারপর ${hub} → ${to} (${l2})  \n`;
                }
            }
            result += `  \n`;
        }
        ...
        if (viaHubsEn.length > 0) {
            ...
            for (const { hub, leg1Modes, leg2Modes, leg1Time, leg2Time } of viaHubsEn) {
                ...
                if (sharedModes.includes('Bus')) {
                    result += `🚗 **By Road:** ${from} → ${hub} (Bus${leg1Time ? ` ~${leg1Time}` : ''}) then ${hub} → ${to} (Bus${leg2Time ? ` ~${leg2Time}` : ''})  \n`;
                }

                if (sharedModes.includes('Train')) {
                    result += `🚂 **By Train:** ${from} → ${hub} (Train${leg1Time ? ` ~${leg1Time}` : ''}) then ${hub} → ${to} (Train${leg2Time ? ` ~${leg2Time}` : ''})  \n`;
                }

                if (leg1Modes.includes('Flight') || leg2Modes.includes('Flight')) {
                    result += `🛫 **By Air:** ${from} → ${hub} (${leg1Modes.join('/')}) then ${hub} → ${to} (${leg2Modes.join('/')})  \n`;
                }
            }
            result += `  \n`;
        }
