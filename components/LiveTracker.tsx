  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 relative">
      ...
      <div className="absolute top-4 left-4 right-4 bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-white p-5 z-10">
        ...
        {globalNearest && (
          <div className="mt-3 mb-3 p-2.5 bg-blue-50 rounded-xl border border-blue-100 flex items-start gap-3">
            <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0 text-blue-600">
              <Compass className="w-4 h-4" />
            </div>
            <div>
              <p className="text-[10px] text-blue-500 font-bold uppercase tracking-wide">Your Current Location</p>
              <p className="text-sm font-bold text-gray-800 leading-tight">
                {globalNearest.station.name}
              </p>
              <p className="text-[10px] text-gray-500 mt-0.5">
                {globalNearest.distance < 100 ? "You are here" : `~${(globalNearest.distance / 1000).toFixed(1)} km away`}
              </p>
            </div>
          </div>
        )}
      ...
      {/* Timeline */}
      <div className="flex-1 overflow-y-auto pt-[22rem] pb-10 px-6">
