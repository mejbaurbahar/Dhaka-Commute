...
const findMetroRoute = (query: string, isBn: boolean): string => {
  ...
  if (foundStations.length >= 2) {
    ...
    return isBn
      ? `🚇 **মেট্রোরেল:** হ্যাঁ, আপনি MRT Line 6 ব্যবহার করে **${s1}** এবং **${s2}** এর মধ্যে যাতায়াত করতে পারেন। সময়: সকাল ৬:৩০ - রাত ৯:৫০ (শুক্রবার বিকাল ২:৩০টায় শুরু)।`
      : `🚇 **Metro Rail:** Yes, you can travel between **${s1}** and **${s2}** using MRT Line 6. Timing: 6:30 AM - 9:50 PM (Friday from 2:30 PM).`;
  }

  const stationList = isBn ? MRT_STATIONS.map(s => s.bn).join(", ") : MRT_STATIONS.map(s => s.en).join(", ");
  return isBn
    ? `🚇 **মেট্রোরেল (MRT Line 6):** উত্তরা উত্তর থেকে মতিঝিল পর্যন্ত চলাচল করে।\n\n**স্টেশনসমূহ:** ${stationList}.\n**সময়:** সকাল ৬:৩০ - রাত ৯:৫০ (শুক্রবার বিকাল ২:৩০ থেকে)।`
    : `🚇 **Metro Rail (MRT Line 6):** Runs from Uttara North to Motijheel.\n\n**Stations:** ${stationList}.\n**Timing:** 6:30 AM - 9:50 PM (Friday from 2:30 PM).`;
};
