...
const ResultCard: React.FC<ResultCardProps> = ({ data }) => {
  ...
  const currentModeStops = useMemo(() => {
    ...
    // List of known major transit points in Bangladesh
    const potentialStops = [
      "Teknaf", "Cox's Bazar", "Chattogram", "Cumilla", "Brahmanbaria", 
      "Bhairab", "Ashuganj", "Mawa", "Padma Bridge", "Barishal", 
      "Khulna", "Jashore", "Sylhet", "Sreemangal", "Tangail", 
      "Sirajganj", "Bogura", "Rangpur", "Aricha", "Paturia", "Daulatdia",
      "Hatiya", "Sandwip", "Bhola"
    ];
    ...
    const matchedStops = potentialStops.filter(stop => 
      ...
      !data.from.includes(stop) && 
      !data.to.includes(stop)
    );

    return Array.from(new Set(matchedStops));
